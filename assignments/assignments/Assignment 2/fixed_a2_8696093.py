...
def addi_test():
    ...
    for i in range(n):
        ...
        question = int(input('What is the sum of %d + %d ? ' % (num1, num2)))

        if question == (num1 + num2):
            score = score + 1

        else:
            print('Sorry that is the incorrect value. The correct answer was: %d' % (num1+num2))
    result = score/n * 100
...
def multi_test():
    ...
    for i in range(n):
        num1 = random.randrange(0,9)
        num2 = random.randrange(0,9)
        question = int(input('What is the multiple of %d * %d ? ' % (num1, num2)))
